...
while(op){
	...
	printf("    	  3. ClearList      9. NextElem          15.SortList\n");
    ...
    switch(op){
	   ...
	   case 2:
		 ...
		 break;
	   case 3:
		 if(ClearList(L)==OK) printf("线性表清理成功！\n");
		     else printf("线性表不存在！\n");       
		 getchar();getchar();
		 break;
	   case 4:
		if(ListEmpty(L)==TRUE) printf("线性表为空！\n");
		 ...
		 break;
	   case 5:
		 if(ListLength(L)!=INFEASIBLE) printf("线性表长度为%d！\n",ListLength(L));
         else printf("线性表不存在！\n");     
		 getchar();getchar();
		 break;
	   case 6:{
	   ...
	   case 8:{
         ...
         else if(PriorElem(L,e,pre)==ERROR) printf("未找到要查找的数的前驱！\n");
         else printf("前驱数为%d!\n",*pre);
		 getchar();getchar();
		 break;}
	   case 9:{
         ...
         printf("您要找哪个数的后驱数？");
         scanf("%d",&e);
         if( PriorElem(L,e,next)==INFEASIBLE)  printf("线性表不存在！\n");
         else if(PriorElem(L,e,next)==ERROR) printf("未找到您要查找的数的后驱！\n");
         else printf("后驱数为%d!\n",*next);
		 getchar();getchar();
		 break;}
	   case 10:{
         ...
         printf("\t请输入您要插入的位置！>>"); 
         scanf("%d", &i);
         if((k = ListInsert(L,i,e)) == INFEASIBLE)  printf("线性表不存在！\n");
	   ...
	   case 12:{
		...
		getchar();getchar();
		 break;}
       case 13:{
           if(MaxSubArray(L)==INFEASIBLE) printf("线性表不存在！\n");
           else if(MaxSubArray(L)==ERROR) printf("线性表是空表！\n");
           else printf("最大连续子数组和为%d",MaxSubArray(L));
           getchar();getchar();
           break;
       }
       case 14:{
           ...
           printf("请输入K：\n");
           scanf("%d",&k);
           if(SubArrayNum(L,k)==INFEASIBLE) printf("线性表不存在！\n");
           else printf("%d",SubArrayNum(L,k));
           getchar();getchar();
           break;
       }
       case 15:{
       ...
       case 20:{
            ...
            if(L.elem == NULL)
                ...
                if(L.length!=0) fprintf(fp, "%d",L.elem[L.length-1]);
                printf("写入完成！\n");
            }
            getchar();getchar();
           break;
...
{
    if(L.elem == NULL) return INFEASIBLE;
    if(i-1 > L.length || i <= 0) return ERROR;
